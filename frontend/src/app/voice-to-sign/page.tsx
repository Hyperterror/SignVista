  ...
  const toggleRecording = async () => {
    if (isRecording) {
      ...
      if (transcript) {
        ...
        toast.success('Voice converted to ISL!');
      }
    } else {
      setIsRecording(true);
      ...
      setOutputSigns([]);
      setTimeout(() => {
        const mockPhrases = ['Hello how are you', 'Thank you very much', 'Nice to meet you', 'Have a great day', 'I am using sign language'];
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              {transcript && (
                <div className="mt-8 w-full max-w-2xl p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg">
                  ...
                  <p className="text-lg text-gray-700 dark:text-gray-300">"{transcript}"</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl border border-gray-200 dark:border-gray-700">
              ...
              <div className="flex flex-wrap gap-6 justify-center mb-8">
                {outputSigns.map((sign, index) => (
                  <div key={index} className={`voice-sign-${index} relative group`}>
                    <div className="w-24 h-24 flex items-center justify-center text-6xl bg-white dark:from-gray-700 dark:to-gray-800 rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:scale-110"
                      style={{ background: 'linear-gradient(135deg, #ffffff, rgba(200, 230, 226, 0.2))' }}
                    >
                      {sign}
                    </div>
                    <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-xs text-gray-600 dark:text-gray-400">{index + 1}</div>
                  </div>
                ))}
              </div>
              <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                {outputSigns.length} signs • Accuracy: 95%
              </div>
            </div>
          </div>
        )}
